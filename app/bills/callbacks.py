...
from app.bills.internal import get_pm_data, email_image
...
year_range = [*range(2008, datetime.now().year)]

DEFAULT_VALUES = {
    'pie_order': 'By Prime Minister',
    'include_other': 'No',
...
def bills_register_dash_components(app):
                    ...
                    dbc.Col(
                        ...
                        dbc.Row([
                            ...
                            dbc.Col(
                                html.Div([
                                    html.Button(
                                        'Remember Visualisation',
                                        id='button-remember-visualisation',
                                        style={
                                    ...
                                    # html.Div(id='remember-container-button', children=html.Br())
                                ], style={
                                    "margin-top": "10px"
                                }),
                                width={"order": 2}
    ...
    def update_graph(pie_order, include_other, time_period_range, size_range):
        ...
        logging.debug(f'{pie_order=}')
        logging.debug(f'{include_other=}')
        logging.debug(f'{time_period_range=}')
        logging.debug(f'{size_range=}')
    ...
    @app.callback(
        Output('remember-container-button', 'children'),
        Input('button-remember-visualisation', 'n_clicks'),
    )
    def remember_fig(n_clicks):
        if not n_clicks:
            return no_update
        # TODO: implement and call function from `internal.py` which sends email to the end user
